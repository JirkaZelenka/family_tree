...
import { loadTemplateVaultFileMap } from '@/lib/data/sample-vault-files'
...
describe('rake edges on template data', () => {
  it('connects married couple with spouse line and compact layout', async () => {
    ...
    expect(p1.displayPlane).toBe('novakovi')
    expect(p13.displayPlane).toBe('novakovi')
    ...
    const coupleSeg = spouseSegs.find(
      (s) =>
